...
#ifdef _WIN64
#define PSFTPDLL L"psftp64.dll"
#define RPSFTPDLL L"\\psftp64.dll"
...
static int __stdcall myProgressProc(int PluginNr, bchar *SourceName,
		bchar *TargetName, int PercentDone) {
...
PsftpMapper::PsftpMapper(bstring const & serverName, bstring const & sessionName, Sftp4tc * cfg) :
	...
	if (!FileExists(dll_2_copy)) {
		MessageBox(gMainWin, PSFTPDLLNF , dll_2_copy, MB_OK);
		return;
	}
	...
	for (int n = 0; n < 999; ++n) {
		...
		dll_2_load += PSFTPDLLU + usedName + no + TEXT(".dll");
	...
	if (!hDll) {
		MessageBox(gMainWin, TEXT("instanced psftp.dll not found"),
				dll_2_load.c_str(), MB_OK);
		return;
	}

	dllName = dll_2_load;

	this->connect = (PsftpConnectProcType) GetProcAddress(hDll,
			"__map__wcplg_open_sftp_session");
	this->disconnect = GetProcAddress(hDll, "__map__wcplg_close_sftp_session");
	this->doSftp = (PsftpDoSftpProcType) GetProcAddress(hDll,
			"__map__wcplg_do_sftp");
	this->getCurrentDirStruct =
			(PsftpGetCurrentDirStructProcType) GetProcAddress(hDll,
					"__map__wcplg_get_current_dir_struct");
	this->freeCurrentDirStruct =
			(PsftpFreeCurrentDirStructProcType) GetProcAddress(hDll,
					"__map__wcplg_free_current_dir_struct");
	this->getLastErrorMessage =
			(PsftpGetLastErrorMessageProcType) GetProcAddress(hDll,
					"__map__wcplg_get_last_error_msg");
	this->initProcs = (PsftpInitProcsProcType) GetProcAddress(hDll,
			"__map__init_Procs");
	this->disconnected = (PsftpDisconnectedProcType) GetProcAddress(hDll,
			"__map__disconnected");
	// new
	this->doConfig = (PsftpDoConfigType) GetProcAddress(hDll,
			"__map__do_config");
	this->getLastAttr = (PsftpGetLastAttrType) GetProcAddress(hDll,
			"__map__get_last_attr");
	this->setTransferAscii = (PsftpSetTransferAscii) GetProcAddress(hDll,
			"__map__set_transfer_ascii");
	this->setConfig = (PsftpSetConfig) GetProcAddress(hDll,
			"__map__set_config");
	this->loadConfig = (PsftpLoadConfig) GetProcAddress(hDll,
			"__map__load_config");

	// enum settings
	this->enumSettingsClose = (PsftpEnumSettingsClose) GetProcAddress(hDll,
			"__map__enum_settings_close");
	this->enumSettingsNext = (PsftpEnumSettingsNext) GetProcAddress(hDll,
			"__map__enum_settings_next");
	this->enumSettingsStart = (PsftpEnumSettingsStart) GetProcAddress(hDll,
			"__map__enum_settings_start");

	// version
	this->getVersion = (PsftpGetVersion) GetProcAddress(hDll,
			"__map__get_version");

	if (this->connect == NULL || this->disconnect == NULL
			|| this->doSftp == NULL || this->getCurrentDirStruct == NULL
			|| this->getLastErrorMessage == NULL || this->initProcs == NULL
			|| this->disconnected == NULL || this->doConfig == NULL
			|| this->getLastAttr == NULL || this->setTransferAscii == NULL
			|| this->setConfig == NULL || this->loadConfig == NULL
			|| this->enumSettingsClose == NULL || this->enumSettingsNext == NULL
			|| this->enumSettingsStart == NULL || this->getVersion == NULL
			|| strcmp(this->getVersion(), PUTTY_VERSION_STRING) != 0) {
		char buff[256];
		if (this->getVersion)
			sprintf(buff, "expected version %s got version %s",
					PUTTY_VERSION_STRING, this->getVersion());
		else
