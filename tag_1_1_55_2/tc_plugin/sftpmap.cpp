...
typedef int  (__stdcall CALLBACK* TD_SFTP_DLL_FNCT_CONNECT)(char*,char*,char* ,int);
typedef int  (__stdcall CALLBACK* TD_SFTP_DLL_FNCT_DO_SFTP)(char*,char*);
typedef my_fxp_names  *(__stdcall CALLBACK* TD_SFTP_DLL_FNCT_GET_CURRENT_DIR_STRUCT)(void);
typedef char*  (__stdcall CALLBACK* TD_SFTP_DLL_FNCT_GLASTERRMSG)(void);
typedef int  (__stdcall CALLBACK* TD_SFTP_DLL_FNCT_init_ProgressProc)			(tProgressProc AP_ProgressProc, int Awc_PluginNr);
typedef void (__stdcall CALLBACK* TD_SFTP_DLL_FNCT_SetSftpServerAccountInfo) (SftpServerAccountInfo ServerAccountInfo);

TD_SFTP_DLL_FNCT_CONNECT SFTP_DLL_FNCT_CONNECT[MAX_Server];
TD_SFTP_DLL_FNCT_DO_SFTP SFTP_DLL_FNCT_DO_SFTP[MAX_Server];
TD_SFTP_DLL_FNCT_GET_CURRENT_DIR_STRUCT SFTP_DLL_FNCT_GET_CURRENT_DIR_STRUCT[MAX_Server];
FARPROC SFTP_DLL_FNCT_DISCONNECT[MAX_Server];
FARPROC SFTP_DLL_FNCT_psftp_memory_hole__stopfen[MAX_Server];
TD_SFTP_DLL_FNCT_GLASTERRMSG SFTP_DLL_FNCT_GET_LAST_ERROR_MESSAGE[MAX_Server];
TD_SFTP_DLL_FNCT_init_ProgressProc SFTP_DLL_FNCT_init_ProgressProc[MAX_Server];
TD_SFTP_DLL_FNCT_SetSftpServerAccountInfo SFTP_DLL_FNCT_SetSftpServerAccountInfo[MAX_Server];
...
int wcplg_sftp_connect(char *user, char *password, char *host, int port, SftpServerAccountInfo *allServers, int CurrentServerId)
{
	...
	*/
	...
	SFTP_DLL_FNCT_CONNECT[CurrentServerId]				=	(TD_SFTP_DLL_FNCT_CONNECT)GetProcAddress(PSFTP_DLL_HANDLER[CurrentServerId],"__map__wcplg_open_sftp_session") ;
	SFTP_DLL_FNCT_DISCONNECT[CurrentServerId]			=	GetProcAddress(PSFTP_DLL_HANDLER[CurrentServerId],"__map__wcplg_close_sftp_session") ;
	SFTP_DLL_FNCT_DO_SFTP[CurrentServerId]				=	(TD_SFTP_DLL_FNCT_DO_SFTP)GetProcAddress(PSFTP_DLL_HANDLER[CurrentServerId],"__map__wcplg_do_sftp") ;
	SFTP_DLL_FNCT_GET_CURRENT_DIR_STRUCT[CurrentServerId]		=	(TD_SFTP_DLL_FNCT_GET_CURRENT_DIR_STRUCT)GetProcAddress(PSFTP_DLL_HANDLER[CurrentServerId],"__map__wcplg_get_current_dir_struct") ;
	SFTP_DLL_FNCT_GET_LAST_ERROR_MESSAGE[CurrentServerId]		=	(TD_SFTP_DLL_FNCT_GLASTERRMSG )GetProcAddress(PSFTP_DLL_HANDLER[CurrentServerId],"__map__wcplg_get_last_error_msg") ;
	SFTP_DLL_FNCT_init_ProgressProc[CurrentServerId]			=	(TD_SFTP_DLL_FNCT_init_ProgressProc)GetProcAddress(PSFTP_DLL_HANDLER[CurrentServerId],"__map__init_ProgressProc") ;
	SFTP_DLL_FNCT_psftp_memory_hole__stopfen[CurrentServerId]	=	GetProcAddress(PSFTP_DLL_HANDLER[CurrentServerId],"__map__psftp_memory_hole__stopfen") ;
	SFTP_DLL_FNCT_SetSftpServerAccountInfo[CurrentServerId]		=	(TD_SFTP_DLL_FNCT_SetSftpServerAccountInfo)GetProcAddress(PSFTP_DLL_HANDLER[CurrentServerId],"__map__set_Server_config_Struct") ;

	if (SFTP_DLL_FNCT_CONNECT[CurrentServerId] == NULL || SFTP_DLL_FNCT_DISCONNECT[CurrentServerId] == NULL || SFTP_DLL_FNCT_DO_SFTP[CurrentServerId] == NULL || SFTP_DLL_FNCT_GET_CURRENT_DIR_STRUCT[CurrentServerId] == NULL || SFTP_DLL_FNCT_GET_LAST_ERROR_MESSAGE[CurrentServerId] == NULL ||SFTP_DLL_FNCT_init_ProgressProc[CurrentServerId]==NULL || SFTP_DLL_FNCT_psftp_memory_hole__stopfen[CurrentServerId] == NULL || SFTP_DLL_FNCT_SetSftpServerAccountInfo[CurrentServerId] == NULL)
	{
	...
	*/
	...
	init_ProgressProc(get_ProgressProc(), get_PluginNumber(), CurrentServerId);
	...
	{  
		RequestProc(PluginNumber,RT_Password ,"Secure FTP: Passphrase for your key","",allServers[CurrentServerId].passphrase,MAX_CMD_BUFFER);
	}

	SFTP_DLL_FNCT_SetSftpServerAccountInfo[CurrentServerId](allServers[CurrentServerId]);
	// Logging
	...
	if (realPort==0) realPort=22;  //last check if port set 2 a valid number
	...
	sprintf(buf_log,"[%s] (connecting to %s@%s:%i using %s)", allServers[CurrentServerId].title, user, 
		realHost, realPort, allServers[CurrentServerId].use_key_auth ? 
		"public key authentication" : "password authentication");
	LogProc_(MSGTYPE_DETAILS,buf_log);

	if ( SFTP_DLL_FNCT_CONNECT[CurrentServerId](user, password, realHost, realPort) == 1)
	{

		if (allServers[CurrentServerId].home_dir != NULL)
		{
			if ((strlen(allServers[CurrentServerId].home_dir) > 0) && (strcmp(allServers[CurrentServerId].home_dir,".")!=0))
			{
	...
	strcpy(buf_log,SFTP_DLL_FNCT_GET_LAST_ERROR_MESSAGE[CurrentServerId]());
	if (strcmp(buf_log,"")!=0) LogProc_(MSGTYPE_IMPORTANTERROR, buf_log); 
	...
	if (allServers[CurrentServerId].is_imported_from_any_datasrc==1 && allServers[CurrentServerId].use_key_auth==1 )
	{
	...
	if (PSFTP_DLL_HANDLER[ServerId] != NULL) // are we really connected ?
	{
		...
		if (cS_ID>-1) 
		{
			...
			if (!_allServer[cS_ID].dont_ask4_passphrase)
				for(unsigned int xx=0;xx<strlen(_allServer[cS_ID].passphrase);xx++) 
					_allServer[cS_ID].passphrase[xx]=0;
			if (!_allServer[cS_ID].dont_ask4_password) _allServer[cS_ID].password[0]='\0';
			if (!_allServer[cS_ID].dont_ask4_username) _allServer[cS_ID].username_cached[0]='\0';
		}
...
int wcplg_sftp_do_commando(char *commando, char *server_output,int ServerId)
{
	char log_buf[MAX_CMD_BUFFER];
	...
	sprintf(log_buf,"%s: %s",_allServer[_CurrentServer_ID].title,commando);

	LogProc_(MSGTYPE_DETAILS,log_buf);

	if (SFTP_DLL_FNCT_DO_SFTP[ServerId](commando,server_output) != 1)
	{
		sprintf(log_buf,"%s: !%s",_allServer[ServerId].title,SFTP_DLL_FNCT_GET_LAST_ERROR_MESSAGE[ServerId]());
...
struct fxp_names *wcplg_sftp_get_current_dir_struct(int ServerId)
{
	...
	// MUST be copied!!!
	my_fxp_names *CurrentDirStruct_TMP=SFTP_DLL_FNCT_GET_CURRENT_DIR_STRUCT[ServerId]();

	if (CurrentDirStruct_TMP != NULL)
	{
		CurrentDirStruct = new fxp_names;
		CurrentDirStruct->nnames = CurrentDirStruct_TMP->nnames;
		CurrentDirStruct->names = (fxp_name*) malloc(sizeof(fxp_name)*CurrentDirStruct_TMP->nnames);

		for(int i=0;i<CurrentDirStruct_TMP->nnames; ++i)
		{
			CurrentDirStruct->names[i].attrs = CurrentDirStruct_TMP->names[i]->attrs;
			CurrentDirStruct->names[i].filename = (char*)malloc(sizeof(char)*(strlen(CurrentDirStruct_TMP->names[i]->filename)+1));
			CurrentDirStruct->names[i].longname = (char*)malloc(sizeof(char)*(strlen(CurrentDirStruct_TMP->names[i]->longname)+1));
			strcpy(CurrentDirStruct->names[i].filename,CurrentDirStruct_TMP->names[i]->filename);
			strcpy(CurrentDirStruct->names[i].longname,CurrentDirStruct_TMP->names[i]->longname);
		}
	}
...
int init_ProgressProc(tProgressProc AP_ProgressProc, int Awc_PluginNr, int ServerId)
{
	if (SFTP_DLL_FNCT_init_ProgressProc[ServerId] != NULL)
	{
		return SFTP_DLL_FNCT_init_ProgressProc[ServerId](AP_ProgressProc,Awc_PluginNr);
	}
	return 0;
}
