...
typedef BOOL (__stdcall * RequestProcType)(int PluginNr, int RequestType,
		bchar *CustomTitle, bchar *CustomText, bchar *ReturnedText, int maxlen);

typedef int (__stdcall * ProgressProcTypeA)(int PluginNr, char *SourceName,
		char *TargetName, int PercentDone);
typedef void (__stdcall * LogProcTypeA)(int PluginNr, int MsgType,
		char const *LogString);
typedef BOOL (__stdcall * RequestProcTypeA)(int PluginNr, int RequestType,
		char *CustomTitle, char *CustomText, char *ReturnedText, int maxlen);

// Function prototypes
int
__stdcall FsInit(int PluginNr, ProgressProcType pProgressProc,
...
int __stdcall FsExtractCustomIcon(bchar* RemoteName, int ExtractFlags,
		HICON* TheIcon);
