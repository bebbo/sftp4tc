...
int sftp_cmd_ls(struct sftp_command *cmd)
{
	...
	while (1) {
	    ...
	    if (names_to_freeing_num<MAX_DIR_FREE_NNAME_CACHE) {
		//MUST be globals for wincmd!!! 
		names_to_freeing[names_to_freeing_num] = ournames[nnames];
		names_to_freeing_num++;	    
	    }
...
int sftp_general_get(struct sftp_command *cmd, int restart)
{
    ...
    char *do_dirty_parse=NULL, *fname_tmp=NULL, *fname_tmp2=NULL, *fname_tmp3;
    double pinc=0;
    double pcb=0;
	...
	if (do_dirty_parse != NULL) {
	    ...
	    //now get the damn reget filname
	    fname_tmp = strchr( cmd->words[1], '?');
	    if (fname_tmp != NULL) {
		int s2cp;
		fname_tmp2 = fname_tmp;
		fname_tmp2++;

		do_dirty_parse-=(fname_tmp-cmd->words[1]);
		fname_tmp3 = (char*)smalloc(sizeof(char)*(strlen(fname)+strlen(cmd->words[1])+1));
		s2cp = strlen(fname)-strlen(do_dirty_parse);
		strncpy(fname_tmp3,fname,s2cp);
		fname_tmp3[s2cp]='\0';
		strcat(fname_tmp3,1+fname_tmp);
		strcat(fname_tmp3,do_dirty_parse);
		sfree(fname);
		fname=fname_tmp3;
	    }
	}
	...
	if (restart) {
	    char *errbuf;
	    errbuf =(char*)smalloc(sizeof(char)*(1000+strlen(fname)+strlen(fxp_error())));
	    strcpy(errbuf,"remote: unable to open ");
	    strcat(errbuf,fname);
	    strcat(errbuf,"\n");
	    strcat(errbuf,fxp_error());

	    MessageBox(
		NULL,					// handle of owner window
		errbuf,				// address of text in message box
		"reget error",		// address of title of message box
		MB_OK   
	    );
	    sfree(errbuf);
	}
	...
	if (restart) {
	    char *errbuf;
	    errbuf =(char*)smalloc(sizeof(char)*(1000+strlen(outfname)));
	    strcpy(errbuf,"local: unable to open ");
	    strcat(errbuf,outfname);
    ...
    if (fxp_fstat_recv(pktin, rreq, &Fattrs) == 1) {
	if (Fattrs.size.lo>0) {			//actually degrading int64 to int32 :-(((
	    pinc = ((double)100/(double)Fattrs.size.lo);
	}
    } else {  
    ...
    sftp_register(req = fxp_close_send(fh));
    //here could come some transfer packets -> skip them
    do {
	rreq = sftp_find_request(pktin = sftp_recv());
    } while (rreq!=req);
    assert(rreq == req);
...
int sftp_general_put(struct sftp_command *cmd, int restart)
{
    ...
    char *fname_tmp=NULL, *fname_tmp3=NULL, *fname_tmp4=NULL, *do_dirty_parse=NULL;
	...
	if (do_dirty_parse != NULL) {
	    char *errbuf;
	    errbuf =(char*)smalloc(sizeof(char)*(1000+strlen(fname)));
	    strcpy(errbuf,"local: unable to open ");
	    strcat(errbuf,fname);
		...
		NULL,			// handle of owner window
		errbuf,			// address of text in message box
		"reput error",		// address of title of message box
		MB_OK   
	    );
	    sfree(errbuf);
	}
	...
	if (do_dirty_parse != NULL) {
	    ...
	    // SegFault chk
	    fname_tmp = fname+strlen(fname)+1;
	    //  fname_tmp == "" oder "/" oder oder "foo/" oder "foo/foo/"
	    if (strlen(fname_tmp)>0) {
		fname_tmp3 = (char*)smalloc(sizeof(char)*(strlen(outfname)+strlen(fname_tmp)+1));
		fname_tmp4 = fname_tmp3;
		strcpy(fname_tmp3,outfname);
		fname_tmp3+=(strlen(outfname)-strlen(fname));
		strcpy(fname_tmp3,fname_tmp);
		strcat(fname_tmp3,fname);

		fname_tmp3 = fname_tmp4;
		sfree(outfname);
		outfname=(char*)smalloc(sizeof(char)*(strlen(fname_tmp3)+1));

		strcpy(outfname,fname_tmp3);
	...
	if (do_dirty_parse != NULL) {
	    char *errbuf;
	    errbuf =(char*)smalloc(sizeof(char)*(1000+strlen(fxp_error()+strlen(outfname))));
	    strcpy(errbuf,"remote: ");
	    strcat(errbuf,outfname);
	    strcat(errbuf," ");
	    strcat(errbuf,fxp_error());
     ...
     * thus put up a progress bar.
     */
    if (stat( fname, &fstat__s ) != -1) {
	//sprintf(tmpbuf_,"%d",fstat__s.st_size);
	//dbg_save_file("c:\\tmp\\1.txt", tmpbuf_);
	f_size_	=	fstat__s.st_size;
    }

    if (f_size_>0) {
	pinc	=	 ((double)100/(double)f_size_);
    } else {
	pinc=50;
    }
...
/*static*/ int psftp_connect(char *userhost, char *user, int portnumber)
{
	...
	*host++ = '\0';
    ...
    strcpy(cfg.proxy_username,&(get_Server_config_Struct().proxy_username[0]));
    strcpy(cfg.proxy_password,&(get_Server_config_Struct().proxy_password[0]));
    strcpy(cfg.proxy_telnet_command,&(get_Server_config_Struct().proxy_telnet_command[0]));
    ...
    if (cfg.ssh_cipherlist[0]==CIPHER_WARN) {
	cfg.ssh_cipherlist[0] = CIPHER_BLOWFISH;
	cfg.ssh_cipherlist[1] = CIPHER_DES;
	cfg.ssh_cipherlist[2] = CIPHER_3DES;
	cfg.ssh_cipherlist[3] = CIPHER_AES;
    }
	...
	if (ssh_sftp_loop_iteration() < 0) {
	    ...
	    wcplg_set_last_error_msg("ssh_init: error during SSH connection setup");
	    return 1;
	}
    }
