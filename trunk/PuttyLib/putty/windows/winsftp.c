...
char *psftp_lcd(char *dir) {
  ...
  if (!SetCurrentDirectory(dir)) {
    ...
    FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL,
        GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR) & message, 0, NULL);
...
RFile *open_existing_file(char *name, uint64 *size, unsigned long *mtime, unsigned long *atime) {
  ...
  h = myCreateFile(name, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, 0);
...
WFile *open_new_file(char *name) {
  ...
  h = myCreateFile(name, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
  if (h == INVALID_HANDLE_VALUE)
    return NULL;
...
WFile *open_existing_wfile(char *name, uint64 *size) {
  ...
  h = myCreateFile(name, GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, 0);
  if (h == INVALID_HANDLE_VALUE)
    return NULL;
...
char *read_filename(DirHandle *dir) {
  do {
    ...
    assert(dir->name);
    if (dir->name[0] == '.' && (dir->name[1] == '\0' || (dir->name[1] == '.' && dir->name[2] == '\0'))) {
      sfree(dir->name);
      dir->name = NULL;
    }
...
WildcardMatcher *begin_wildcard_matching(char *name) {
  ...
  *last = '\0';
  if (fdat.cFileName[0] == '.'
      && (fdat.cFileName[1] == '\0' || (fdat.cFileName[1] == '.' && fdat.cFileName[2] == '\0')))
    ret->name = NULL;
  else
...
char *wildcard_get_filename(WildcardMatcher *dir) {
  while (!dir->name) {
    ...
    if (fdat.cFileName[0] == '.' && (fdat.cFileName[1] == '\0' || (fdat.cFileName[1] == '.' && fdat.cFileName[2]
        == '\0')))
      dir->name = NULL;
    else
...
int do_eventsel_loop(HANDLE other_event) {
  ...
  if ((unsigned) (n - WAIT_OBJECT_0) < (unsigned) nhandles) {
    ...
    i = 0;
    for (socket = first_socket(&socketstate); socket != INVALID_SOCKET; socket = next_socket(&socketstate))
      i++;
    ...
    for (i = 0; i < skcount; i++) {
      ...
      if (!p_WSAEnumNetworkEvents(socket, NULL, &things)) {
        static const struct {
          int bit, mask;
        } eventtypes[] = { { FD_CONNECT_BIT, FD_CONNECT }, { FD_READ_BIT, FD_READ }, { FD_CLOSE_BIT, FD_CLOSE }, {
            FD_OOB_BIT, FD_OOB }, { FD_WRITE_BIT, FD_WRITE }, { FD_ACCEPT_BIT, FD_ACCEPT }, };
        int e;
...
char *ssh_sftp_get_cmdline(char *prompt, int no_fds_ok) {
  ...
  if ((sftp_ssh_socket == INVALID_SOCKET && no_fds_ok) || p_WSAEventSelect == NULL) {
