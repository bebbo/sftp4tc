...
static int s_wrpkt_prepare(Ssh ssh, struct Packet *pkt, int *offset_p)
{
	...
	log_packet(ssh->logctx, PKT_OUTGOING, pkt->data[12],
		   ...
		   pkt->body, pkt->length - (int)(pkt->body - pkt->data),
...
{
    ssh_pkt_adddata(pkt, data, (int)strlen(data));
			 ...
			 struct Packet *pktin)
{
	...
	rsa_fingerprint(logmsg + (int)strlen(logmsg),
			sizeof(logmsg) - (int)strlen(logmsg), &s->hostkey);
	logevent(logmsg);
    }
	...
	if (s->publickey_blob && !s->tried_publickey) {
		...
		ret = loadrsakey(s->keyfile, &s->key, passphrase,
				 &error);
		if (passphrase) {
		    smemclr(passphrase, (int)strlen(passphrase));
		    sfree(passphrase);
		}
		if (ret == 1) {
		    /* Correct passphrase. */
	...
	while (1) {
		...
		while (pktin->type == SSH2_MSG_USERAUTH_PASSWD_CHANGEREQ) {
			...
			if (!ret) {
			    ...
			    /* burn the evidence */
			    free_prompts(s->cur_prompt);
			    smemclr(s->password, (int)strlen(s->password));
			    sfree(s->password);
			    ssh_disconnect(ssh, NULL, "Unable to authenticate",
					   SSH2_DISCONNECT_AUTH_CANCELLED_BY_USER,
					   TRUE);
