					  ...
					  int protocol, int protcfginfo)
{
	...
	*/
	...
	ssd->cancelbutton = ctrl_pushbutton(s, "Close", 'c', HELPCTX(no_help),
		sessionsaver_handler, P(ssd));
	...
	*/
	...
	if (have_backend(PROT_SSH) && (!midsession || protocol == PROT_SSH)) {
		...
		* Again, we adjust the tab order to reflect this. */
	    ...
	    ctrl_settitle(b, "SFTP4TC",
			  "Options controlling SFTP4TC");
	    ...
	    ctrl_checkbox(s, "hide .names", 'd',
			P("SFTP4TC:.names"),
			  dlg_stdcheckbox_handler,I(offsetof(Config,sftp4tc) + offsetof(struct Sftp4tc, hideDotNames)));
	    ctrl_editbox(s, "default chmod", 'm', 50,
			 P("SFTP4TC:defChMod"),
			 dlg_stdeditbox_handler, I(offsetof(Config,sftp4tc) + offsetof(struct Sftp4tc, defChMod)),
			 I(sizeof(((Config *)0)->sftp4tc.defChMod)));
	    ctrl_editbox(s, "exe chmod", 'e', 50,
			 P("SFTP4TC:exeChMod"),
			 dlg_stdeditbox_handler, I(offsetof(Config,sftp4tc) + offsetof(struct Sftp4tc, exeChMod)),
			 I(sizeof(((Config *)0)->sftp4tc.exeChMod)));
	    ctrl_editbox(s, "exe extensions", 'x', 100,
