...
int sftp_general_get(struct sftp_command *cmd, int restart)
{
  ...
  if (restart && fname) {
    ...
    if (do_dirty_parse != NULL) {
      ...
      if (fname_tmp != NULL) {
        ...
        fname_tmp3 =
          (char *) smalloc(sizeof(char) *
                           (strlen(fname) + strlen(cmd->words[1]) + 1));
  ...
  if (!fh) {
    if (restart) {
      ...
      errbuf =
        (char *) smalloc(sizeof(char) *
                         (1000 + strlen(fname) + strlen(fxp_error())));
    ...
    while (xfer_download_data(xfer, &vbuf, &len)) {
      ...
      while (wpos < len) {
        if (mode)
        {
          ...
          int i=wpos, start=wpos;
          while ((i<len) && (buf[i]!=10)) ++i;
          if (i<len) {
            fwrite(buf + start, 1, i-start, fp);
            fwrite(buf_1310, 1, 2, fp);
          } else {
            fwrite(buf + start, 1, i-start, fp);
          }
          wlen=i+1-start;
...
int sftp_general_put(struct sftp_command *cmd, int restart)
{
  ...
  char *fname_tmp = NULL, *fname_tmp3 = NULL, *fname_tmp4 =
    NULL, *do_dirty_parse = NULL;
  ...
  if (restart) {
    if (do_dirty_parse != NULL) {
      ...
      if (strlen(fname_tmp) > 0) {
        fname_tmp3 =
          (char *) smalloc(sizeof(char) *
                           (strlen(outfname) + strlen(fname_tmp) + 1));
        ...
        sfree(outfname);
        outfname =
          (char *) smalloc(sizeof(char) * (strlen(fname_tmp3) + 1));
  ...
  if (!fh) {
    ...
    if (do_dirty_parse != NULL) {
      ...
      errbuf =
        (char *) smalloc(sizeof(char) *
                         (1000 + strlen(fxp_error() + strlen(outfname))));
    ...
    while (xfer_upload_ready(xfer) && !err && !eof) {
      ...
      if (len == -1) {
        ...
        if (mode)
          ...
          while (i<len) {
            while ((i<len) && (buffer[i]!=13)) i++;
            if ((i<len) && (buffer[i]==13)) {
              if (i+1>len)
...
int psftp_main(int argc, char *argv[])
{
  ...
  for (i = 1; i < argc; i++) {
    ...
    ret =
      cmdline_process_param(argv[i], i + 1 < argc ? argv[i + 1] : NULL, 1,
                            &cfg);
    if (ret == -2) {
