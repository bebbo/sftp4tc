...
static void not_connected(void) {
	nprintf(("psftp: not connected to a host; use \"open host.name\"\n"));
	set_disconnected();
}
...
int sftp_get_file(char *fname, char *outfname, int recurse, int restart) {
	...
	if (recurse) {
		...
		if (result && (attrs.flags & SSH_FILEXFER_ATTR_PERMISSIONS)
			...
			if (file_type(outfname) != FILE_TYPE_DIRECTORY
				...
				nprintf(("%s: Cannot create directory\n", outfname));
				return 0;
			}
			 ...
			 */
			{
				...
				for (; i < nnames; i++) {
					...
					if (outfname)
						...
						nextoutfname = dupstr(ournames[i]->filename);
					ret = sftp_get_file(nextfname, nextoutfname, recurse,
							restart);
	...
	if (restart) {
		...
		if (seek_file(file, uint64_make(0, 0), FROM_END) == -1) {
			close_wfile(file);
			nprintf(("reget: cannot restart %s - file too large\n", outfname));
			req = fxp_close_send(fh);
			pktin = sftp_wait_for_reply(req);
			fxp_close_recv(pktin, req);
			return 0;
		...
		nprintf(("reget: restarting at file position %s\n", decbuf));
	} else {
		offset = uint64_make(0, 0);
	}

	nprintf(("remote:%s => local:%s\n", fname, outfname));
	...
	while (!xfer_done(xfer)) {
		...
		while (ret && xfer_download_data(xfer, &vbuf, &len)) {
			...
			while (wpos < len) {
				...
				if (wlen <= 0) {
					nprintf(("error while writing local file\n"));
					ret = 0;
					xfer_set_error(xfer);
					break;
				}
...
int sftp_put_file(char *fname, char *outfname, int recurse, int restart) {
	...
	if (recurse && file_type(fname) == FILE_TYPE_DIRECTORY) {
		...
		if (!dh) {
			nprintf(("%s: unable to open directory\n", fname));
			return 0;
		}
	...
	if (restart) {
		...
		if (!ret) {
			close_rfile(file);
			nprintf(("read size of %s: %s\n", outfname, fxp_error()));
			return 0;
		}
		if (!(attrs.flags & SSH_FILEXFER_ATTR_SIZE)) {
			close_rfile(file);
			nprintf(("read size of %s: size was not given\n", outfname));
			return 0;
		}
		offset = attrs.size;
		uint64_decimal(offset, decbuf);
		nprintf(("reput: restarting at file position %s\n", decbuf));
	...
	while (!err && (!eof || !xfer_done(xfer))) {
		...
		while (xfer_upload_ready(xfer) && !err && !eof && !userAbort) {
			len = read_from_file(file, buffer, sizeof(buffer) - 1);
			if (len == -1) {
				nprintf(("error while reading local file\n"));
				err = 1;
			} else if (len == 0) {
...
SftpWildcardMatcher *sftp_begin_wildcard_matching(char *name) {
	...
	if (!check) {
		nprintf(("Multiple-level wildcards are not supported\n"));
		sfree(unwcdir);
		return NULL;
	}
...
int sftp_cmd_unknown(struct sftp_command *cmd) {
	nprintf(("psftp: unknown command \"%s\"\n", cmd->words[0]));
	return 0; /* failure */
}
...
int sftp_cmd_cd(struct sftp_command *cmd) {
	...
	sfree(pwd);
	pwd = dir;
	nprintf(("Remote directory is now %s\n", pwd));
...
int sftp_cmd_mkdir(struct sftp_command *cmd) {
	...
	if (cmd->nwords < 2) {
		nprintf(("mkdir: expects a directory\n"));
		return 0;
	}
...
int sftp_cmd_rmdir(struct sftp_command *cmd) {
	...
	if (cmd->nwords < 2) {
		nprintf(("rmdir: expects a directory\n"));
		return 0;
	}
...
int sftp_cmd_mv(struct sftp_command *cmd) {
	...
	if (cmd->nwords < 3) {
		nprintf(("mv: expects two filenames\n"));
		return 0;
	}
	...
	if ((cmd->nwords > 3 || is_wildcard(cmd->words[1])) && !ctx->dest_is_dir) {
		nprintf(
				("mv: multiple or wildcard arguments require the destination" " to be a directory\n"));
		sfree(ctx->dstfname);
		return 0;
	}
...
static int sftp_action_chmod(void *vctx, char *fname) {
	...
	if (!result || !(attrs.flags & SSH_FILEXFER_ATTR_PERMISSIONS)) {
		nprintf(
				("get attrs for %s: %s\n", fname, result ? "file permissions not provided" : fxp_error()));
		return 0;
	}
...
int sftp_cmd_chmod(struct sftp_command *cmd) {
	...
	if (cmd->nwords < 3) {
		nprintf(("chmod: expects a mode specifier and a filename\n"));
		return 0;
	}
	...
	if (mode[0] >= '0' && mode[0] <= '9') {
		if (mode[strspn(mode, "01234567")]) {
			nprintf(
					("chmod: numeric file modes should" " contain digits 0-7 only\n"));
			return 0;
		}
		ctx->attrs_clr = 07777;
		...
		while (*mode) {
			...
			if (!*mode || *mode == ',') {
				nprintf(
						("chmod: file mode '%.*s' is incomplete\n", (int)strcspn(modebegin, ","), modebegin));
				return 0;
			}
			action = *mode++;
			if (!*mode || *mode == ',') {
			...
			while (*mode && *mode != ',') {
				...
				default:
					nprintf(
							("chmod: file mode '%.*s' contains unrecognised" " permission specifier '%c'\n", (int)strcspn(modebegin, ","), modebegin, *mode));
					return 0;
				}
				mode++;
			}
...
static int sftp_cmd_open(struct sftp_command *cmd) {
	...
	if (back != NULL) {
		nprintf(("psftp: already connected\n"));
		return 0;
	}

	if (cmd->nwords < 2) {
		nprintf(("open: expects a host name\n"));
		return 0;
	}
	...
	if (cmd->nwords > 2) {
		...
		if (portnumber == 0) {
			nprintf(("open: invalid port number\n"));
			return 0;
		}
	} else
		portnumber = 0;
...
static int sftp_cmd_lcd(struct sftp_command *cmd) {
	...
	if (cmd->nwords < 2) {
		nprintf(("lcd: expects a local directory name\n"));
		return 0;
	}

	errmsg = psftp_lcd(cmd->words[1]);
	if (errmsg) {
		nprintf(("lcd: unable to change directory: %s\n", errmsg));
		sfree(errmsg);
		return 0;
	}

	currdir = psftp_getcwd();
	nprintf(("New local directory is %s\n", currdir));
	sfree(currdir);
...
static int sftp_cmd_lpwd(struct sftp_command *cmd) {
	...
	currdir = psftp_getcwd();
	nprintf(("Current local directory is %s\n", currdir));
	sfree(currdir);
	...
	int (*obey)(struct sftp_command *);
		...
		/* FIXME: this example is crap for non-Windows. */
				...
				{ "del", TRUE, "delete files on the remote server",
						...
						sftp_cmd_rm }, { "delete", FALSE, "del", NULL,
						sftp_cmd_rm },
				{ "dir", TRUE, "list remote files",
						...
						sftp_cmd_ls }, { "exit", TRUE, "bye", NULL,
						sftp_cmd_quit },
				{ "get", TRUE,
				...
				{ "pwd", TRUE, "print your remote working directory",
						...
						sftp_cmd_pwd }, { "quit", TRUE, "bye", NULL,
						sftp_cmd_quit },
				{ "reget", TRUE, "continue downloading files",
						...
						sftp_cmd_reget },
				{ "ren", TRUE, "mv", NULL, sftp_cmd_mv }, { "rename", FALSE,
						"mv", NULL, sftp_cmd_mv },
...
static int sftp_cmd_help(struct sftp_command *cmd) {
	...
	if (cmd->nwords == 1) {
		...
		for (i = 0; i < sizeof(sftp_lookup) / sizeof(*sftp_lookup); i++) {
			...
			nprintf(("%-*s", maxlen+2, lookup->name));
		...
		for (i = 1; i < cmd->nwords; i++) {
			...
			if (!lookup) {
				nprintf(("help: %s: command not found\n", cmd->words[i]));
			} else {
				nprintf(("%s", lookup->name));
				if (lookup->longhelp == NULL)
					lookup = lookup_command(lookup->shorthelp);
				nprintf(("%s", lookup->longhelp));
			}
		}
	}
	return 1;
...
struct sftp_command * line2command(char * line, int mode, int modeflags) {
	char *p, *q, *r;
	int quoting;
...
struct sftp_command *sftp_getcmd(FILE *fp, int mode, int modeflags) {
	...
	cmd = line2command(line, mode, modeflags);
...
int do_sftp_init(void) {
	...
	if (!homedir) {
		fprintf(stderr, "Warning: failed to resolve home directory: %s\n",
				fxp_error());
		homedir = dupstr(".");
	} else {
		nprintf(("Remote working directory is %s\n", homedir));
	}
	pwd = dupstr(homedir);
	return 0;
}
...
void do_sftp(int mode, int modeflags, char *batchfile) {
	...
	if (mode == 0) {
		...
		if (!fp) {
			nprintf(("Fatal: unable to open %s\n", batchfile));
			return;
		}
		while (1) {
			struct sftp_command *cmd;
...
static void usage(void) {
	nprintf(("PuTTY Secure File Transfer (SFTP)) client\n"));
	nprintf(("%s\n", ver));
	nprintf(("Usage: psftp [options] [user@]host\n"));
	nprintf(("Options:\n"));
	nprintf(("  -V        print version information and exit\n"));
	nprintf(("  -pgpfp    print PGP key fingerprints and exit\n"));
	nprintf(("  -b file   use specified batchfile\n"));
	nprintf(("  -bc       output batchfile commands\n"));
	nprintf(("  -be       don't stop batchfile processing if errors\n"));
	nprintf(("  -v        show verbose messages\n"));
	nprintf(("  -load sessname  Load settings from saved session\n"));
	nprintf(("  -l user   connect with specified username\n"));
	nprintf(("  -P port   connect to specified port\n"));
	nprintf(("  -pw passw login with specified password\n"));
	nprintf(("  -1 -2     force use of particular SSH protocol version\n"));
	nprintf(("  -4 -6     force use of IPv4 or IPv6\n"));
	nprintf(("  -C        enable compression\n"));
	nprintf(("  -i key    private key file for authentication\n"));
	nprintf(("  -noagent  disable use of Pageant\n"));
	nprintf(("  -agent    enable use of Pageant\n"));
	nprintf(("  -batch    disable all interactive prompts\n"));
	cleanup_exit(1);
}
...
int psftp_connect(char *userhost, char *user, int portnumber) {
	...
	if (host == NULL) {
		...
		*host++ = '\0';
		if (user) {
			nprintf(
					("psftp: multiple usernames specified; using \"%s\"\n", user));
		} else
			user = userhost;
	}
	...
	if (!loaded_session) {
		...
		struct Sftp4tc c2;
		memcpy(&c2, &cfg, sizeof(c2));
		c2.config = conf_new();
		conf_set_str(c2.config, CONF_host, "");
		do_defaults(host, &c2);
		if (conf_get_str(c2.config, CONF_host)[0] != '\0') {
	...
	if (strlen(cfg.sftpCommand) > 0) {
		conf_set_str(conf, CONF_remote_cmd, cfg.sftpCommand);
	 ...
	 */
	conf_set_str(conf, CONF_remote_cmd2, "test -x /usr/lib/sftp-server &&"
	...
	err = back->init(NULL, &backhandle, conf, conf_get_str(conf, CONF_host),
			conf_get_int(conf, CONF_port), &realhost, 0,
			conf_get_int(conf, CONF_tcp_keepalives));
	if (err != NULL) {
		fnprintf((stderr, "ssh_init: %s\n", err));
		return 1;
	}
	logctx = log_init(NULL, cfg.config);
...

	{
		...
		while (!back->sendok(backhandle)) {
			...
			if (ssh_sftp_loop_iteration() < 0) {
				fnprintf(
						(stderr, "ssh_init: error during SSH connection setup\n"));
				return 1;
			}
		}
	}
	if (verbose && realhost != NULL)
		nprintf(("Connected to %s\n", realhost));
	if (realhost != NULL)
		sfree(realhost);
	return 0;
}
...
int psftp_main(int argc, char *argv[]) {
	...
	for (i = 1; i < argc; i++) {
		...
		ret = cmdline_process_param(argv[i], i + 1 < argc ? argv[i + 1] : NULL,
				1, conf);
		if (ret == -2) {
