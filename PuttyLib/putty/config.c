...
{
	int button;
	struct Sftp4tc * sftp4tc = (struct Sftp4tc *)data; Conf *conf = (Conf *)sftp4tc->config;
...
static void config_host_handler(union control *ctrl, void *dlg,
				void *data, int event)
{
	struct Sftp4tc * sftp4tc = (struct Sftp4tc *)data;
	Conf *conf = (Conf *)sftp4tc->config;
...
static void inipath_handler(union control *ctrl, void *dlg,
							void *data, int event)
{
	struct Sftp4tc *cfg = (struct Sftp4tc *)data;
	if (event == EVENT_REFRESH) {
		int i, sel;
		char * p;
		dlg_update_start(ctrl, dlg);
		dlg_listbox_clear(ctrl, dlg);
		dlg_listbox_addwithid(ctrl, dlg, "", -1);
		sel = 0;
		for(i = 0;; ++i) {
...
static int load_selected_session(struct sessionsaver_data *ssd,
				 void *dlg, struct Sftp4tc *conf, int *maybe_launch)
{
...
static void sessionsaver_handler(union control *ctrl, void *dlg,
				 void *data, int event)
{
	struct Sftp4tc *conf = (struct Sftp4tc *)data;
    struct sessionsaver_data *ssd =
	(struct sessionsaver_data *)ctrl->generic.context.p;

	if (ssd->savedsession)
		strncpy(theSelectedSession, ssd->savedsession, 512);
	...
	if (ctrl == ssd->editbox) {
		...
		if (*ssd->savedsession && !inSelect) {
				int mbl = FALSE;
				inSelect = 1;
				dlg_listbox_select_string(ssd->listbox, dlg, ssd->savedsession);
				inSelect = 0;
		}
	    dlg_update_done(ctrl, dlg);
	}
	...
	if (!ssd->midsession &&
		...
		(mbl && ctrl == ssd->listbox && conf_launchable(conf->config))) {
		...
		del_settings(ssd->sesslist.sessions[i], conf);
		get_sesslist(&ssd->sesslist, FALSE);
		get_sesslist(&ssd->sesslist, TRUE);
		dlg_refresh(ssd->listbox, dlg);
	    }
		...
		!conf_launchable(conf->config)) {
			struct Sftp4tc c2;
			int imbl = FALSE;
			memcpy(&c2, conf, sizeof(c2));
			c2.config = conf_new();
			if (!load_selected_session(ssd, dlg, &c2, &imbl)) {
				dlg_beep(dlg);
				conf_free(c2.config);
				return;
			}
		/* If at this point we have a valid session, go! */
					if (mbl && conf_launchable(c2.config)) {
						conf_copy_into(conf->config, c2.config);
		    dlg_end(dlg, 1);
		} else
		    dlg_beep(dlg);
		      ...
		      int protocol, int protcfginfo)
{
	...
	ctrl_settitle(b, "SFTP4TC Options", "Options for the SFTP4TC plugin");
	s = ctrl_getset(b, "SFTP4TC Options", "sftp4tc", "SFTP4TC Options");
			ctrl_editbox(s, "home dir", 'm', 100,
				HELPCTX(no_help),
				conf_editbox_handler, I(CONF_sftpHomeDir), I(1));
			ctrl_checkbox(s, "cache folders", 'f',
				HELPCTX(no_help),
				conf_checkbox_handler,I(CONF_sftpCacheFolders));
			ctrl_checkbox(s, "hide . names", 't',
				HELPCTX(no_help),
				conf_checkbox_handler,I(CONF_sftpHideDotNames));
			ctrl_editbox(s, "default chmod", 'd', 50,
				HELPCTX(no_help),
				conf_editbox_handler, I(CONF_sftpDefChMod), I(1));
			ctrl_editbox(s, "exe chmod", 'e', 50,
				HELPCTX(no_help),
				conf_editbox_handler, I(CONF_sftpExeChMod), I(1));
			ctrl_editbox(s, "exe extensions (.sh)", 'x', 100,
				HELPCTX(no_help),
				conf_editbox_handler, I(CONF_sftpExeExtensions), I(1));
