...
#define CONFIG_OPTIONS(X) \
    ...
    X(INT, NONE, crhaslf) \
    X(STR, NONE, winclass) \
	X(INT, NONE, sftpCacheFolders) \
	X(INT, NONE, sftpHideDotNames) \
	X(STR, NONE, sftpDefChMod) \
	X(STR, NONE, sftpExeChMod) \
	X(STR, NONE, sftpExeExtensions) \
...
size_t conf_serialised_size(Conf *conf);
