...
{
	sftp_pkt_adddata(pkt, data, (int)strlen(data));
...
static struct fxp_xfer *xfer_init(struct fxp_handle *fh, uint64 offset)
{
	...
	xfer->req_maxsize = 32768 * NPACKETS; // prefetch NPACKETS
    xfer->err = 0;
...
int xfer_tail_complete(struct fxp_xfer *xfer)
{
	return xfer->tail->complete;
}
