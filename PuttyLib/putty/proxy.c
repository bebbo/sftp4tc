	...
	if (p->state == PROXY_STATE_NEW) {
		...
		*/
		...
		if (username[0] || password[0]) {
			...
			for (i = 0, j = (int)(buf2); i < len; i += 3, j += 4)
	...
	if (p->state == PROXY_CHANGE_NEW) {
		...
		*/
		...
		if (type == ADDRTYPE_IPV6) {
			...
			namelen = (int)strlen(hostname) + 1;   /* include the NUL */
			addr[0] = addr[1] = addr[2] = 0;
			addr[3] = 1;
		}

		username = conf_get_str(p->conf, CONF_proxy_username);
		length = (int)strlen(username) + namelen + 9;
		...
		if (p->state == 2) {
			...
			*/
			...
			if (type == ADDRTYPE_IPV4) {
				...
				command[4] = (int)strlen(command+5);
		...
		if (fmt[so] == '\\') {
			...
			*/
			...
			if (fmt[eo] == '%') {
				...
				int userlen = (int)strlen(username);
				ENSURE(userlen);
