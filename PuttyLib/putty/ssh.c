...
extern struct ssh2_userkey *sshcom_read(const Filename *filename, char *passphrase);
extern struct ssh2_userkey *openssh_read(const Filename *filename, char *passphrase);
...
static int do_ssh1_login(Ssh ssh, unsigned char *in, int inlen, int ispkt)
{
  ...
  char *keystr;
    ...
    ptr = (unsigned char *)ssh_pkt_getdata(ssh, 8);
    if (!ptr) {
	...
	cancel_connection = verify_ssh_host_key(ssh->frontend,
			    ssh->savedhost, ssh->savedport, "rsa", keystr,
			    fingerprint);
	sfree(keystr);
	if (cancel_connection == 1) {
	    wcplg_set_last_error_msg("connection canceled because you do not trust the server - additional: the Server-Connection is using ssh1 Protocol");
	    bombout(("_"));
	    crReturn(0);
	}

	//STOP HERE couse its ssh1
	wcplg_set_last_error_msg("connection canceled - Server is using ssh1 Protocol which does not support the sftp Protocol");
	bombout(("_"));
	crReturn(0);
	exit(0);
    }
...
static int do_ssh2_transport(Ssh ssh, unsigned char *in, int inlen, int ispkt)
{
  ...
  begin_key_exchange:
    ...
    if (cancel_connection == 1) return -1;
...
static int ssh2_try_send(struct ssh_channel *c)
{
	...
	dont_log_data(ssh, PKTLOG_OMIT);
	ssh2_pkt_addstring_start(ssh);
	ssh2_pkt_addstring_data(ssh, (char *)data, len);
	end_log_omission(ssh);
	ssh2_pkt_send(ssh);
...
static void do_ssh2_authconn(Ssh ssh, unsigned char *in, int inlen, int ispkt)
{
  ...
  auth_type type;
	...
	if (!filename_is_null(ssh->cfg.keyfile)) {
	    ...
	    } else if ((keytype == SSH_KEYTYPE_OPENSSH) || (keytype == SSH_KEYTYPE_SSHCOM))  {
		/*
		just let it be for now;
		*/
		s->publickey_blob = NULL;
		s->tried_agent = 1; //just don't try that agent
		if (!s->method) s->method = AUTH_PUBLICKEY_FILE; //skip to publickey :-) , if no method was choosed
	    } else {
		char *msgbuf;
	...
	while (1) {
		...
		if (!ssh2_pkt_getbool(ssh)) {
			...
			c_write_str(ssh, "Access denied\r\n");
			wcplg_set_last_error_msg("Access denied");
	    ...
	    if ((keytype == SSH_KEYTYPE_OPENSSH) || (keytype == SSH_KEYTYPE_SSHCOM)) {
		if (!pk_file_tried) s->method = AUTH_PUBLICKEY_FILE;
		else s->method = (auth_method)0;
		pk_file_tried++;
	    } else s->method = (auth_method)0;
	    ssh->pkt_ctx &= ~SSH2_PKTCTX_AUTH_MASK;
	    s->need_pw = FALSE;
		...
		if (keytype==SSH_KEYTYPE_OPENSSH)
		    key = openssh_read(&ssh->cfg.keyfile, get_Server_config_Struct()->passphrase);
		else if (keytype==SSH_KEYTYPE_SSHCOM)
		    key = sshcom_read(&ssh->cfg.keyfile, get_Server_config_Struct()->passphrase);
		else	//in the beginning was a check if it is a correct key...
		    key = ssh2_load_userkey(&ssh->cfg.keyfile, get_Server_config_Struct()->passphrase, &error);

		for(xx=0; xx<strlen(get_Server_config_Struct()->passphrase); ++xx)
			get_Server_config_Struct()->passphrase[xx]=0;
