...
#include "puttyver.h"
   ...
   */
  void getCodePage(config_tag * c) {
    CpData * cp = &CPDATA[0];
    // search all code pages
    while (cp->name) {
      char * p = &c->line_codepage[0];
      char * q = cp->name;
      while (*q) {
  ...
  config_tag * __stdcall __map__wcplg_open_sftp_session(char *user, char *password, char *host, int port) {
    config_tag * c;
    ISinitT = 0;
	if (user) cfg.sftp4tc.isUnicode = 1;
  ...
  int __stdcall __map__init_Procs(tRequestProcType AP_RequestProc, tProgressProc AP_ProgressProc, int Awc_PluginNr,
      HWND hwnd) {
  ...
  struct config_tag * __stdcall __map__do_config(HWND hwnd, int midsession, int protocol) {
    config_tag * c = do_config(hwnd, midsession, protocol);
    if (c)
      getCodePage(c);
    return c;
  }
