...
extern wchar_t * toWideChar(char * str);
...
static void sftp_pkt_addstring_data(struct sftp_packet *pkt, char *data, int len) {
...
static int sftp_pkt_getattrs(struct sftp_packet *pkt, struct fxp_attrs *ret) {
  ...
  if (ret->flags & SSH_FILEXFER_ATTR_UIDGID) {
    if (!sftp_pkt_getuint32(pkt, &ret->uid) || !sftp_pkt_getuint32(pkt, &ret->gid))
      return 0;
  }
  ...
  if (ret->flags & SSH_FILEXFER_ATTR_ACMODTIME) {
    if (!sftp_pkt_getuint32(pkt, &ret->atime) || !sftp_pkt_getuint32(pkt, &ret->mtime))
      return 0;
  }
  if (ret->flags & SSH_FILEXFER_ATTR_EXTENDED) {
    ...
    while (count--) {
       ...
       * we recognise.
       */
      if (!sftp_pkt_getstring(pkt, &str, &len) || !sftp_pkt_getstring(pkt, &str, &len))
        return 0;
    }
  }
  return 1;
...
static int fxp_got_status(struct sftp_packet *pktin) {
  ...
  "unexpected OK response", "end of file", "no such file or directory", "permission denied", "failure", "bad message",
      "no connection", "connection lost", "operation unsupported", };
...
struct fxp_names *fxp_readdir_recv(struct sftp_packet *pktin, struct sftp_request *req) {
  ...
  if (pktin->type == SSH_FXP_NAME) {
     ...
     */
    if (!sftp_pkt_getuint32(pktin, &i) || i > (pktin->length - pktin->savedpos) / 12) {
    ...
    for (i = 0; i < (unsigned long) ret->nnames; i++) {
      char *str1, *str2;
      int len1, len2;
      if (!sftp_pkt_getstring(pktin, &str1, &len1) || !sftp_pkt_getstring(pktin, &str2, &len2) || !sftp_pkt_getattrs(
          pktin, &ret->names[i].attrs)) {
        fxp_internal_error("malformed FXP_NAME packet");
        while (i--) {
          sfree(ret->names[i].filename);
          sfree(ret->names[i].longname);
          sfree(ret->names[i].ucFilename);
        }
        sfree(ret->names);
        sfree(ret);
        sfree(pktin);
      ...
      ret->names[i].ucFilename = toWideChar(ret->names[i].filename);
    }
    sftp_pkt_free(pktin);
    return ret;
  } else {
...
struct fxp_name *fxp_dup_name(struct fxp_name *name) {
  ...
  ret->ucFilename = dupwstr(name->ucFilename);
...
int xfer_download_gotpkt(struct fxp_xfer *xfer, struct sftp_packet *pktin) {
  ...
  if (rr->retlen < rr->len) {
    uint64 filesize = uint64_add32(rr->offset, (rr->retlen < 0 ? 0 : rr->retlen));
#ifdef DEBUG_DOWNLOAD
    { char buf[40];
